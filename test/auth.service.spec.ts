...
describe('AuthService', () => {
  ...
  describe('registerUniversity', () => {
    it('should have registerUniversity method', () => {
      expect(service.registerUniversity).toBeDefined();
      expect(typeof service.registerUniversity).toBe('function');
    });

    // Note: Full registerUniversity flow is tested in backend.integration.spec.ts
    // because it requires database transactions, which are better tested with a real DB
    // or proper transaction mock that mirrors the actual QueryRunner behavior.
  });
});
